...
def test_all():
    ...
    chennai = data["cities"]["chennai"]
    
    all_chennai_routes = []
    for category, routes in chennai["routes"].items():
        for r in routes:
            all_chennai_routes.append(r)
            
    print(f"Loaded {len(all_chennai_routes)} Chennai routes.")
    
    passed = 0
    failed = 0
    failed_details = []
    
    for r in all_chennai_routes:
        rid = r["route_id"]
        rnum = r["route_number"]
        name = r["name"]
        stops = r["stops"]
    ...
    print(f"\n--- TEST SUMMARY ---")
    print(f"Total Routes Tested: {len(all_chennai_routes)}")
    ...
    if failed > 0:
        ...
        print("\n🎉 All Chennai routes verified successfully!")
        sys.exit(0)
